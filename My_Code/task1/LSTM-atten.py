#!/usr/bin/env python
# coding: utf-8
# Best F1 score  [0.5834523469484909, 30]
...
def save(epoch):
    if not os.path.exists(os.path.join(CWD,'model_LSTM_atten')):
        os.makedirs(os.path.join(CWD,'model_LSTM_atten'))
    torch.save(model.state_dict(), os.path.join( CWD,'model_LSTM_atten/model.pkl.'+str(epoch) ))
    with open( os.path.join( CWD,'model_LSTM_atten/history.json'), 'w') as f:
        json.dump(history, f, indent=4)


# In[ ]:

# AttentionModel (batch_size, output_size, hidden_size, vocab_size, embedding_length, weights)
model = AttentionModel(batch_size, 6, hidden_dim, max_words, embedding_dim, embedding_matrix)
...
with open(os.path.join(CWD,'model_LSTM_atten/history.json'), 'r') as f:
...
plt.legend()
plt.show()
plt.savefig("Loss_LSTM_atten.png")
...
plt.legend()
plt.show()
plt.savefig("F1_score__atten.png")
...
model.load_state_dict(state_dict=torch.load(os.path.join(CWD,'model_LSTM_atten/model.pkl.{}'.format(best_model))))
model.train(False)
...
SubmitGenerator(prediction,
                ...
                True, 
                os.path.join(CWD,'submission_LSTM_atten.csv'))
