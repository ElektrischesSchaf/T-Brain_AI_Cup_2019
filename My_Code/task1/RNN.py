#!/usr/bin/env python
# coding: utf-8
# Best F1 score  [0.6037416405654786, 8]
...
def save(epoch):
    if not os.path.exists(os.path.join(CWD,'model_RNN')):
        os.makedirs(os.path.join(CWD,'model_RNN'))
    torch.save(model.state_dict(), os.path.join( CWD,'model_RNN/model.pkl.'+str(epoch) ))
    with open( os.path.join( CWD,'model_RNN/history.json'), 'w') as f:
        json.dump(history, f, indent=4)


# In[ ]:

# RNN (batch_size, output_size, hidden_size, vocab_size, embedding_length, weights)
model = RNN(batch_size, 6, hidden_dim, max_words, embedding_dim, embedding_matrix)
...
with open(os.path.join(CWD,'model_RNN/history.json'), 'r') as f:
...
model.load_state_dict(state_dict=torch.load(os.path.join(CWD,'model_RNN/model.pkl.{}'.format(best_model))))
model.train(False)
